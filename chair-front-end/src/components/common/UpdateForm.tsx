...
export const UpdateForm = ({type, id, closeModal, element, setData}) => {
    ...
    const  handleSubmit = async (e) => {
        ...
        axios.put(`http://localhost:8080/api/${type}/${id}`, payload)
            .then(res => res.data)
            .then(val => {
                setData(prev => prev.map(el => el.id === val?.id ? val : el))
                closeModal()
            }).catch(err => alert(err))
